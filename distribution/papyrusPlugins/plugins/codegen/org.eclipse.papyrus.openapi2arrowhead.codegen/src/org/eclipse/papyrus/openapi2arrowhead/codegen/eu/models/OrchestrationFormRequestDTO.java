...
public class OrchestrationFormRequestDTO implements Serializable {
	...
	public OrchestrationFlags orchestrationFlags = new OrchestrationFlags();
	...
	//-------------------------------------------------------------------------------------------------
	public OrchestrationFormRequestDTO validateCrossParameterConstraints() throws Exception {
        if (requestedService == null && orchestrationFlags.get(Flag.OVERRIDE_STORE)) {
            throw new Exception("Requested service can not be null when \"" + Flag.OVERRIDE_STORE + "\" is TRUE");
        }
        
        if (requestedService == null && orchestrationFlags.get(Flag.TRIGGER_INTER_CLOUD)) {
            throw new Exception("Requested service can not be null when \"" + Flag.TRIGGER_INTER_CLOUD + "\" is TRUE");
        }
        ...
        if (orchestrationFlags.get(Flag.ONLY_PREFERRED)) {
            ...
            if (preferredProviders.isEmpty()) {
                throw new Exception("There is no valid preferred provider, but \"" + Flag.ONLY_PREFERRED + "\" is set to true");
            }
        }
        ...
        if (orchestrationFlags.get(Flag.ENABLE_QOS) && commands.containsKey(QOS_COMMAND_EXCLUSIVITY) && !orchestrationFlags.get(Flag.MATCHMAKING)) {
        	throw new Exception("Exclusive service orchestration is only possible when \"" + Flag.MATCHMAKING + "\" is set to true");
        }
	...
	public static class Builder {
		...
		//-------------------------------------------------------------------------------------------------
		public OrchestrationFormRequestDTO build() throws Exception {
